...
const App: React.FC = () => {
  ...
  const joinRoom = async () => {
    ...
    if (gameState.players[0].id === user.uid) return; // Already in
  ...
  const playTurn = async (cardIndex: number) => {
    ...
    state.logs = [`${player.name}님이 ${card.month}월 카드를 플레이했습니다.`, ...state.logs].slice(0, 5);
      ...
      <div className="h-screen flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-lg p-8 rounded-2xl border border-white/20 w-full max-w-md shadow-2xl">
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            <input 
              type="email" placeholder="이메일" className="w-full bg-black/30 p-4 rounded-xl border border-white/10" 
              value={email} onChange={e => setEmail(e.target.value)}
            />
            <input 
              type="password" placeholder="비밀번호" className="w-full bg-black/30 p-4 rounded-xl border border-white/10"
              value={password} onChange={e => setPassword(e.target.value)}
            />
            <button className="w-full py-4 bg-red-600 hover:bg-red-700 rounded-xl font-bold transition">로그인 / 회원가입</button>
          </form>
          ...
          <button onClick={handleGoogleLogin} className="w-full py-4 bg-white text-black rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-gray-100 transition">
            <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" className="w-6 h-6" /> Google로 시작하기
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center p-4 gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl">
            <button onClick={createRoom} className="p-10 bg-green-600/20 border border-green-500/30 rounded-3xl hover:bg-green-600/30 transition flex flex-col items-center gap-4 group">
  ...
  // Join if second player
  if (gameState && gameState.players[1].id === "waiting" && user.uid !== gameState.players[0].id) {
    joinRoom();
  }
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden relative select-none">
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-20">
         <div className="flex gap-4">
            <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-3">
               <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-xs font-bold">상대</div>
               <div>
                  <div className="text-[10px] text-gray-400">{opp.name}</div>
         ...
         <div className="flex items-center gap-2 bg-black/50 px-6 py-2 rounded-full border border-white/20">
            ...
            <span className="font-bold">{isMyTurn ? '내 차례' : '상대 대기 중...'}</span>
         </div>
      ...
      <div className="h-24 flex justify-center items-start pt-16 opacity-50">
      ...
      {/* Captured Areas */}
      <div className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col gap-4 max-h-[60%] overflow-y-auto w-40">
         <div className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">상대 획득</div>
         <div className="flex flex-wrap gap-1">
            {opp.captured.map((c, i) => <HanafudaCard key={i} card={c} isSmall />)}
         </div>
      </div>

      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-4 max-h-[60%] overflow-y-auto w-40 items-end">
         <div className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">내 획득</div>
      ...
      {/* Game Board */}
      <div className="flex-1 flex items-center justify-center px-48 py-24">
         <div className="w-full h-full max-w-5xl bg-white/5 rounded-[4rem] border border-white/10 grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 p-8 items-center justify-center overflow-y-auto">
            {gameState.floor.map((card) => (
               <HanafudaCard key={card.id} card={card} className="animate-deal" />
            ))}
            {gameState.floor.length === 0 && (
                <div className="col-span-full text-center text-white/20 font-black text-2xl">바닥에 패가 없습니다</div>
            )}
         </div>
      </div>

      {/* My Hand */}
      <div className={`h-48 flex flex-col items-center justify-end pb-8 transition-transform ${!isMyTurn ? 'translate-y-12 opacity-50 grayscale' : 'translate-y-0'}`}>
         <div className="text-sm font-bold text-yellow-400 mb-2">내 점수: {me.score}점</div>
         <div className="flex gap-2">
            {me.hand.map((card, i) => (
               <HanafudaCard 
                key={card.id} 
                card={card} 
                onClick={isMyTurn ? () => playTurn(i) : undefined}
                className={isMyTurn ? 'hover:shadow-yellow-400/50' : ''}
               />
            ))}
         </div>
      </div>
      ...
      <div className="absolute bottom-4 left-4 max-w-xs space-y-1 pointer-events-none">
         {gameState.logs.map((log, i) => (
            <div key={i} className={`text-[10px] p-2 rounded bg-black/40 backdrop-blur-sm transition-opacity ${i === 0 ? 'opacity-100' : 'opacity-40'}`}>
               {log}
            </div>
         ))}
      </div>
      ...
      {gameState.isGameOver && (
          <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center animate-in fade-in duration-500">
              <div className="text-center space-y-6">
                  <h2 className="text-6xl font-black italic text-yellow-500 drop-shadow-lg">
                      {gameState.winner === user.uid ? '대 승 리 !' : '패 배 . . .'}
                  </h2>
                  <button 
                    onClick={() => { setRoomId(null); window.history.pushState({}, '', '/'); }}
                    className="px-10 py-4 bg-red-600 rounded-full font-bold hover:scale-105 transition"
                  >
                    로비로 돌아가기
                  </button>
              </div>
